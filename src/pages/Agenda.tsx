      ...
      if (!finalPacoteId) {
        ...
        if (tipoAtendimentoRascunho === "Plano") {
          ...
          if (!planoSelecionado || !pacotePlanoSelecionado) {
            toast.error("Por favor, selecione o Plano de Saúde e o Pacote de Repasse.");
            return;
          }
          ...
          if (errPacPlano) throw new Error("Erro ao criar pacote financeiro: " + errPacPlano.message);
          ...
          if (errPacote) throw new Error("Erro ao criar pacote particular: " + errPacote.message);
                  ...
                  <>
                    ...
                    {tipoAtendimentoRascunho === "Particular" ? (
                      ...
                      <div className="space-y-3 p-3 border rounded-lg bg-blue-50/30">
                        ...
                        <div className="space-y-1.5">
                          <Label>Pacote de Repasse (Valor)</Label>
                          <Select value={pacotePlanoSelecionado} onValueChange={setPacotePlanoSelecionado} required>
                            <SelectTrigger>
                              <SelectValue placeholder="Selecione o pacote/regra..." />
                            </SelectTrigger>
                            <SelectContent>
                              {listaPacotes.map(p => <SelectItem key={p.id} value={p.id}>{p.nome} (R$ {p.preco_total})</SelectItem>)}
                            </SelectContent>
                          </Select>
                        </div>
